...
void resetTerminal(){
    if (G.is_raw){
        if (tcsetattr(STDINF, TCSAFLUSH, &G.savedTerm) == -1){
            PERRO("%s", "Erro ao restaurar o terminal");
            exit(1);
        }
...
int getTerminalSize(int *cols, int *rows){
    ...
    if (ioctl(STDOUTF, TIOCGWINSZ, &ws) == -1 || ws.ws_col == 0)
        return 0;
    G.width = *cols = ws.ws_col;
    G.height = *rows = ws.ws_row;
...
void setMouseEvents(int mouse_opt){
    if (!G.is_raw){
        PERRO("%s", "Terminal deve estar em modo raw para habilitar eventos  de mouse");
        exit(1);
    }
...
int getEvent(struct Event *event){
    ...
    if (!G.is_raw)
        KILL("%s", "Terminal precisa estar em modo raw para capturar eventos");
...
void exit_raw(int s){
    ...
    clearScreen(FULL);
    moveCursor(1 + G.width / 2 - msg_sz / 2, 1 + G.height / 2);
    printf("%s\n", msg);
